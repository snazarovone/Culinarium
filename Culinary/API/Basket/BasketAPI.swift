...
class BasketAPI{
    static func requstObjectBasket <T>(type: T.Type, request: BasketServerAPI, delegate:AppDelegate, callback: @escaping (T?, Bool)->()) where T : BaseResponseModel{
        
        delegate.providerBasketServerAPI.manager.session.getAllTasks { (value) in
        ...
        delegate.providerBasketServerAPI.rx.request(request).mapObject(T.self).asObservable().subscribe(onNext: { (responce) in
